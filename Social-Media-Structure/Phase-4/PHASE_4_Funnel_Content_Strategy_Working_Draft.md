...
**Status:** Phase 4 strategy approved and complete for planning; Seul approved the content approach integration and final strategy closeout on September 24, 2026. This document compiles steps 1–6 discussed in chat. Strategy completion does not certify operational readiness, authorize production or publication, activate an account, approve offers or prove that a destination works.  
...
```mermaid
flowchart TD
    C["Cold audience"] --> D["Discovery"]
    W["Prior engagement"] --> I["Interest or Trust"]
    R["Referral or warm introduction"] --> T["Trust or Intent"]
    D --> I
    I --> T
    T --> N["Intent"]
    I --> S["Social Signal Check"]
    N --> S
    ...
    O --> U["Useful follow-up, redirect or decline"]
...
```mermaid
flowchart TD
    Q["Human fit decision"] --> A["Ready and appropriate"]
    Q --> N["Fit; not ready"]
    Q --> X["Outside scope"]
    N --> I["Relevant explanation or agreed follow-up"]
    I --> Q
    X --> R["Redirect or close"]
    A --> C["Agreed scope and payment"]
    C --> D["Delivery and reporting"]
...
## Content approach application; optional planning field

Phase 3.3 defines the approach as how CMS engages an audience with an idea. In Phase 4, choose a primary approach only when it clarifies the audience's current question and the content job. A direction may progress through several stages while its approach changes; no approach is required to fill a mix or stage quota. The approach is a planning tag, not a public heading, script or new content format. Select it from actual source material and an accountable human perspective.

The following strand illustrates the **primary Western growth-stage owner-led service business**. It is hypothetical planning, not a CMS client case or verified buyer observation. Its premise is that content decisions depend on the owner's approval; confirm the situation through a real source before adapting it into audience content.

| Phase 4 stage | Audience need | Content direction | Primary approach; Phase 3.3 format | CTA and destination; planning only |
|---|---|---|---|---|
| Discovery | Recognize why posting stalls | The owner is still the final stop for every content decision | **Relatable**; Diagnostic Breakdown | Reflect on where approvals stop; an explicit CTA is optional; CMS content/profile awaiting validation |
| Interest | Examine possible causes | A posting gap could come from approval ownership, capacity or unclear direction | **Disruptor**; Diagnostic Breakdown | Explore a bounded explanation or preliminary Social Signal Check; route planned |
| Trust | Inspect CMS's reasoning | Show how CMS would distinguish an approval bottleneck from a shortage of ideas | **Showcase**; Proof-of-Method Demonstration, explicitly illustrative | Examine CMS's method; explanation awaiting validation |
| Intent | Choose the right depth of help | Explain when a process adjustment, bounded review or broader support may fit | **Authority**; Decision Framework | Consider a Signal-to-Strategy Review if the situation is specific; route planned |
| Qualification | Know what context to provide | Explain what CMS needs to understand before recommending support | **Handraiser**; FAQ or Process Walkthrough | Submit context through a tested intake route for human fit review; route currently awaiting validation |

These rows describe a possible sequence, not a required linear journey. Discovery names a symptom; Interest opens possible causes without diagnosing; Trust shows method, not client results; Intent compares routes without prescribing a package; Qualification requires a human disposition, not merely a submission. For this example, **Personal Take** could be a separate, genuinely held and reviewed founder or strategist perspective on why CMS asks about decision ownership. **Consumable** is a presentation note, such as making the distinction quick to understand without losing necessary qualifications; it is not a primary approach. Evidence, destination status and release gates in the matrix below still apply.
...
| Content; audience | Stage and intended purpose | Pillar; format | Primary approach; optional | Evidence or permission | CTA | Destination; planning status | Owner role; not assigned | Countable event and next decision |
|---|---|---|---|---|---|---|---|---|
| Fragmentation or owner burden; G, PH | Discovery; relevant recognition | P3; POV/Diagnostic Breakdown | Relatable for G; PH unassigned | Grounding; PH example checked | Attention/Reflection | Content/profile; **awaiting validation** | Content/community | Relevant response; explain further? |
...
| Symptom versus constraint; G, PH | Interest; understand causes | P1 + P2; Decision Framework/Diagnostic Breakdown | Disruptor for G; PH unassigned | Reasoned possibilities, not diagnosis | Education/Self-assessment | Resource or Social Signal Check; **planned** | Content/route | Resource action and check start/completion separately; clarify need? |
...
| CMS diagnostic reasoning; G, E, PH | Trust; examine method | P4 + P5; Process/Method demonstration | Showcase for G; E/PH unassigned | Documented method; illustrative label | Method | Observed homepage explanation; **awaiting validation** | Evidence/destination | Method interaction or question; concern resolved? |
...
| Diagnostic route explanation; G, PH | Intent; choose next step | P4; FAQ/Decision Framework | Authority for G; PH unassigned | Current scope, exclusions, pricing status, local assumptions | Review/Diagnostic | Review/Intensive qualification; **planned** | Commercial/destination | Specific route request; appropriate fit route? |
...
| Intake expectations; G, E, PH | Qualification; prepare context | P4; Process/FAQ | Handraiser for G; E/PH unassigned | Accurate intake and response ownership | Review/Diagnostic | Observed homepage form; **awaiting validation** | Human intake/backup | Submission, receipt, first human reply, disposition separately |
...
## Phase 4 strategy closeout; September 24, 2026

Seul approved the eight stage definitions, optional routes, audience variants, reconciled qualification definition and planning matrix. She approved adding Phase 3.3's optional primary approach to the matrix and the illustrative audience strand above, and explicitly approved Phase 4 as complete for strategy planning. The Phase 2.5.10 planning handoff and Phase 3.9 Green planning decision are recorded; Phase 3.10 is an approved planning framework. This closeout applies to the planning document and takes effect in the repository on merge. It does not complete the operational activation checks below. No live test, final price or platform launch is implied by approval of this planning document.
...
**Current outcome:** Phase 4 strategy approved and complete for planning as of September 24, 2026; the qualification conflict is resolved and the optional content approach field is integrated. Operational activation, commercial validation, production and publication remain separate decisions.
